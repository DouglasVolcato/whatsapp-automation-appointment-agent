import { AppointmentEntity } from "../../domain/abstract/entities/appointment-entity";
...
export class AppointmentRepositorty extends Repository {
  ...
  public async getManyByDay(input: {
    day: string;
  ...
  public async getAvailableHours(input: {
    day: string;
  }): Promise<{ hour: string }[]> {
    const query = `
      WITH all_hours AS (
  ...
  public async checkIfDatetimeIsAvailable(input: {
    date_time: string;
    ...
    `;
    return (
      await this.executeSql({ query: query, params: [input.date_time] })
    )[0];
  }

  public async getManyByUser(input: {
    user_id: string;
  }): Promise<AppointmentEntity[]> {
